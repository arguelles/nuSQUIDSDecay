...
int main(int argc, char** argv){
  bool oscillogram = false;
  bool quiet = true;
  bool print_expected_spectrum = false;
...
//  std::string flux_path = "/home/carguelles/work/NeutrinoDecay/verosimilitud/data/HondaGaisser.h5";
  ...
  std::string mc_path = "/home/mmoulai/work/NeutrinoDecay/IC86SterileNeutrinoDataRelease/monte_carlo/NuFSGenMC_nominal.dat";
  ...
  // const std::string modelname = "CombinedGHandHG_H3a_SIBYLL2.3_rc1_pl";  
  // const std::string modelname = "CombinedGHandHG_H3a_QGSJET-II-04";
   //const std::string modelname = "PolyGonato_SIBYLL2.3_rc1_pl";
   //const std::string modelname = "ZatsepinSokolskaya_pamela_QGSJET-II-04";
  //const std::string modelname = "ZatsepinSokolskaya_pamela_SIBYLL2.3_rc1_pl";
  double m1 = 0.0;
...
*/
  ...
  //nusquids_pion->Set_Majorana(true);
  //nusquids_kaon->Set_Majorana(true);

  //nusquids_pion->Set_EvalThreads(1);
  //nusquids_kaon->Set_EvalThreads(1);

  nusquids_kaon->Set_TauRegeneration(true);
  nusquids_pion->Set_TauRegeneration(true);

  // set mixing angles and masses
  //nusquids_kaon->Set_MixingAngle(0,1,0.55766355);
  //nusquids_kaon->Set_MixingAngle(0,2,0.14717912);
  //nusquids_kaon->Set_MixingAngle(1,2,0.67580512);
  ...
  //nusquids_kaon->Set_MixingAngle(0,1,0.0);
  //nusquids_kaon->Set_MixingAngle(0,2,0.0);
  //nusquids_kaon->Set_MixingAngle(1,2,0.0);
  ...
  //nusquids_pion->Set_MixingAngle(0,1,0.55766355);
  //nusquids_pion->Set_MixingAngle(0,2,0.14717912);
  //nusquids_pion->Set_MixingAngle(1,2,0.67580512);
  ...
  //nusquids_pion->Set_MixingAngle(0,1,0.0);
  //nusquids_pion->Set_MixingAngle(0,2,0.0);
  //nusquids_pion->Set_MixingAngle(1,2,0.0);
  ...
  if(oscillogram){
    ...
    std::ofstream ioutput("/home/mmoulai/osc_ra/initial_" + std::to_string(nu3mass) + "_" + std::to_string(theta24) + "_" + std::to_string(lifetime) + ".dat");
  ...
  if(oscillogram){
    ...
    const unsigned int neutrino = 0;
    const unsigned int antineutrino = 1;   
    std::ofstream output("/home/mmoulai/osc_ra/final_" + std::to_string(nu3mass) + "_" + std::to_string(theta24) + "_" + std::to_string(lifetime) + ".dat");
    for(double costh : nusquids_kaon->GetCosthRange()){
      for(double enu : nusquids_kaon->GetERange()){
      //for(double log10E = 3.0; log10E < 6.0; log10E +=  0.001 ){
        // const double enu = pow(10.0,log10E);
  ...
  auto V = Verosimilitud(numneu, data_path.c_str(), flux_path.c_str(), effective_area_path.c_str(),
                         mc_path, nusquids_kaon, nusquids_pion);

  // starting point
  std::vector<double> nuis_param {1.0, 0.01, 1.0, 1.0, 1.0, -0.03};

  // minimizer settings
  std::vector<bool> param_to_minimize {true, true, true, true, true, true};
  std::vector<double> low_bound {0.001, -1.0, 0.0, 0.0, 0.91,  -0.5};
  std::vector<double> high_bound {2.0,  1.0, 2.0, 2.0, 1.1978, 0.5};

  //std::vector<double> low_bound {0.08, 0.0, 0.8, 0.968, 0.944,  -0.088};
  //std::vector<double> high_bound {1.8, 0.25, 1.6, 1.005, 1.016, -0.024};
  ...
  if(print_expected_spectrum){
    std::cout << std::endl;
    std::cout << "V.GetPertExpectationVec" << std::endl;
    std::cout << std::endl;
    std::vector<double> nuisance (min_ret.begin(), min_ret.end()-1);
    std::cout<<" Minimized nuisance:   ";
    for (auto i = nuisance.begin(); i != nuisance.end(); ++i)
	std::cout<< *i << ' ';
    std::cout << std::endl;
    std::cout << std::endl;
    std::cout << "Expected distribution @ minimized nuisance: " << std::endl;
    std::vector<double> expvec = V.GetPertExpectationVec(nuisance);
    for (auto i = expvec.begin(); i != expvec.end(); ++i)
       std::cout << *i << ", ";
    std::cout << std::endl;
    std::cout << std::endl; 
    
    std::vector<double> nuis_prior {1.0, 0.0, 1.0, 1.0, 1.0, 0.0};
    std::cout<< " Nuisance prior:   ";
    for (auto i = nuis_prior.begin(); i != nuis_prior.end(); ++i)
       std::cout<< *i << ",";
    std::cout << std::endl;
    std::cout << std::endl;
    std::cout << "Expected distribution @ nuisance prior: " << std::endl;
    std::vector<double> priorvec = V.GetPertExpectationVec(nuis_prior); 
    for (auto i = priorvec.begin(); i != priorvec.end(); ++i)
        std::cout << *i << ",";
    std::cout << std::endl; 
 }
  ...
  if(print_data){
    std::cout << std::endl;
    std::cout<< "V.GetDataVec" << std::endl;
    std::vector<double> datavec = V.GetDataVec();
